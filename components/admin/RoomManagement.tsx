...
export default function RoomManagement() {
  ...
  const [editingRoom, setEditingRoom] = useState<Room | null>(null); // Track which room is being edited
  ...
  // 1. Fetch rooms from Neon on mount
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingRoom) {
      ...
      if (result.success && result.room) {
        setRooms(
          rooms.map((r) => (r.id === editingRoom.id ? result.room! : r)),
        );
        closeModal();
      }
    } else {
    ...
    <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 relative">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm italic border-l-2 border-blue-600 pl-2">
